...
export default function Tweet({navigation,details,authuser,likesinfo,sharesinfo,commentsinfo}) {
    const{colors}=useTheme()
    const db=getFirestore(app)
    const [readmore,setreadmore]=React.useState(true)
    let desc=details?.data.tweet
    ...
    const addsharefunction=async()=>{
            ...
            shs.push(authuser)
            await updateDoc(doc(db,"tweetshares",sharesinfo[0].id),{
                    shares:shs
                })
            sendPushNotification(details?.data.userinfo.token,`${userinfo.username} shared your fleet`)
    ...
    const addlikefunction=async()=>{
        setloading(true)
        const docRef = doc(db, "tweetlikes", likesinfo[0]?.id);
        ...
        lks.push(authuser)
        await updateDoc(doc(db,"tweetlikes",likesinfo[0].id),{
                likes:lks
            })
        sendPushNotification(details?.data.userinfo.token,`${userinfo.username} liked your fleet`)    
    ...
    const deletefunc=()=>{
        setloading(true)
        deleterecordfunction("tweets",details?.id)
        deleterecordfunction("tweetlikes",details?.id)
        deleterecordfunction("tweetcomments",details?.id)
        deleterecordfunction("tweetshares",details?.id)
        setTimeout(() => {
            setloading(false)
            alert("Fleet has been deleted")
...
<View
   ...
   <View style={{
    ...
    <TouchableOpacity onPress={()=>navigation.navigate("comments",{cmts:commentsinfo[0],owner:details?.data.userinfo,dbname:"tweetcomments"})} style={{marginRight:RFPercentage(.5)}}>
